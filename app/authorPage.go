...
func HandleAuthorRequest(w http.ResponseWriter, r *http.Request) {
	idStr := strings.TrimPrefix(r.URL.Path, "/Author/")
	...
	AuthorIdHandler(w, r, id)
}

func AuthorIdHandler(w http.ResponseWriter, r *http.Request, id_Cat int) {
	var posts []Post1
	...
	rows, err := db.Query(query, id_Cat)
	title_page := ""
	if err != nil {
		fmt.Println(err)
		return
	...
	if err != nil {
		...
		RenderTemplate(w, "postsAuthorId.html", data)
		return
	}
